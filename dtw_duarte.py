...
def dtw_sw(x, y, winlen, alpha=0.5, **kwargs):
    ...
    for i in range(xl):
        for j in range(yl):
            ...
            # No window selected
            if window is None:
                tmp_ac[i, j] = sliding_dist(nx[int(pad_i - (winlen / 2)):int(pad_i + (winlen / 2)) + 1],
                                        ny[int(pad_j - (winlen / 2)):int(pad_j + (winlen / 2)) + 1],
                                        dnx[int(pad_i - (winlen / 2)):int(pad_i + (winlen / 2)) + 1],
                                        dny[int(pad_j - (winlen / 2)):int(pad_j + (winlen / 2)) + 1], alpha, swindow)
...
def plot_alignment(ref_signal, estimated_signal, path, **kwargs):
    ...
    plt.plot(copy_ref, color=sns.color_palette()[0], lw=linewidths[0], label='reference')
    plt.plot(estimated_signal, color=sns.color_palette()[1], lw=linewidths[1], label='estimate')
    plt.legend(fontsize=17)

    [plt.plot([[path[0][i]], [path[1][i]]],
